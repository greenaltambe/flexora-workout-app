...
setWorkoutError(
err.response?.data?.message || "Failed to load workout recommendations"
);
} finally {
...
console.log("Logging workout with data:", workoutData);
...
{workoutData.exercise_recommendations?.map((exercise, index) => (
<motion.div
key={index}
...
<h2 className="text-3xl font-bold text-white">Meal Ideas</h2>
</div>
...
<MdRestaurant className="mx-auto mb-4 text-gray-600" size={64} />
<p className="text-lg">Select a meal type to view suggestions</p>
</div>
)}
</motion.section>
</motion.div>
